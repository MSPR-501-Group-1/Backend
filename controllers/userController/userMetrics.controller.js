...
// Get fitness analytics metrics
export const getFitnessMetrics = async (req, res) => {
    try {
        const { range } = req.query;
        const result = await userMetricsService.getFitnessMetrics(range);
        ...
        console.error("Erreur getFitnessMetrics:", error);
        res.status(500).json({
            success: false,
            message: "Erreur lors de la récupération des metrics fitness"
        });
    }
};
